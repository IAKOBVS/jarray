...
#define f_jarr_cat_noalloc(jarr_ptr, argc, ...) JARR_GENERIC(jarr_cat_noalloc, jarr_ptr, argc, __VA_ARGS__)

#define JARR_CAT_NOCHECK(T, t)                                              \
ALWAYS_INLINE int jarr_cat_nocheck_##T(T *RESTRICT jarr_ptr, int argc, ...) \
{                                                                           \
	size_t tmpCap = jarr_ptr->capacity;                                 \
	do { tmpCap *= 2; } while (jarr_ptr->size + argc > tmpCap);         \
	return jarr_reserve_nocheck_##T(jarr_ptr, tmpCap)                   \
	&& (jarr_ptr->capacity = tmpCap, 1);                                \
}

JARR_TEMPLATE_T_t(JARR_CAT_NOCHECK)

#define f_jarr_cat_nocheck(jarr_ptr, argc, ...) JARR_GENERIC(jarr_cat_nocheck, jarr_ptr, argc, __VA_ARGS__)
