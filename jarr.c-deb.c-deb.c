...
static __attribute__((always_inline)) inline int private_jarr_realloc(
    void **restrict jarr, size_t size) {
  void *tmp;
  return ((tmp = realloc(*jarr, size)) ? (*jarr = tmp, 1) : 0);
}
static __attribute__((always_inline)) inline void
private_jarr_grow_cap_while_lt_size(size_t size, size_t *restrict cap) {
  do {
    *cap *= 2;
  } while (size > *cap);
}
